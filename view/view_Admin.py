...
class View_Admin:
    ...
    def Main_Admin(self):
        ...
        self.button_User_Level = tk.Button(self.navbar, text="Nivel de Usuario", bg="blue", fg="white", command=lambda:Validaciones.back_window(self.Main_User_Level, self.window))
        self.button_User_Level.pack(side="left", padx=10, pady=5)
        ...
        self.Main_Admin_Content(self.contenido)
...
# Apartado User_Level
    # Main_User_Level -> ventana principal de opciones de control de niveles de usuario
    def Main_User_Level(self):
        # instancia de controladores
        self.control_User_Login = control.Usuario_LoginC()
        ...
        self.button_id = tk.Button(self.navbar, text="Buscar ID", bg="blue", fg="white", command=self.Form_Search_User_Level)
        self.button_id.pack(side="left", padx=10, pady=5)

        self.button_update = tk.Button(self.navbar, text="Actualizar", bg="blue", fg="white", command=self.Form_Update_User_Level)
        self.button_update.pack(side="left", padx=10, pady=5)

        self.button_create = tk.Button(self.navbar, text="Agregar", bg="blue", fg="white", command=self.Form_Add_User_Level)
        self.button_create.pack(side="left", padx=10, pady=5)

        self.button_delete = tk.Button(self.navbar, text="Eliminar", bg="blue", fg="white", command=self.Form_Delete_User_Level)
        ...
        # fill table
        for row in self.control_User_Login.get_all_user_access_Controller():
...
# Formulario para buscar registro por id
    def Form_Search_User_Level(self):
        ...
        data_user = []
        for user in self.control_User_Login.get_all_user_access_Controller():
    ...
    def Query_Search_User_Level(self):
        data_user_level = self.control_User_Login.get_one_user_access_Controller(self.entry_user_id.get().split()[0])[0]
...
# Formulario para agregar sesiones se usuarios
    def Form_Add_User_Level(self):
        self.top_level = tk.Toplevel(self.window)
        self.top_level.title("Agregar Acceso")
        self.top_level.grab_set()

        self.label_user_name = tk.Label(self.top_level, text="Nombre de Usuario:")
        self.label_user_name.grid(row=0, column=0, padx=10, pady=5)
        self.entry_user_name = tk.Entry(self.top_level, validate="key", validatecommand=self.validate_str)
        self.entry_user_name.grid(row=0, column=1, padx=10, pady=5)

        self.label_password = tk.Label(self.top_level, text="Contraseña:")
        self.label_password.grid(row=1, column=0, padx=10, pady=5)
        self.entry_password = tk.Entry(self.top_level, validate="key", validatecommand=self.validate_str)
        ...
        #Asignar usuarios al combobox
        data_rol = []
        for rol in self.control_Rol.get_all_roles_Controller():
            data = f"{rol[0]} - {rol[1].split()[0]}"
            data_rol.append(data)

        self.label_rol_id = tk.Label(self.top_level, text="ID de Rol:")
        self.label_rol_id.grid(row=3, column=0, padx=10, pady=5)
        self.entry_rol_id = ttk.Combobox(self.top_level, values=data_rol, state="readonly")
        self.entry_rol_id.grid(row=3, column=1, padx=10, pady=5)

        button_add = tk.Button(self.top_level, text="Agregar", command=self.Query_Add_User_Level)
        button_add.grid(row=4, columnspan=2, padx=10, pady=10)

    def Query_Add_User_Level(self):
        if self.entry_user_name.get() == "" and self.entry_password == "":
            messagebox.showwarning("Advertencia de Sesion de Usuario", "Debe llenar todos los formularios")
            return
        else:
            if self.control_User_Login.insert_user_access_Controller(self.entry_user_name.get(),  self.entry_password.get(), self.entry_user_id.get().split()[0], self.entry_rol_id.get().split()[0]) != None:
...
# Formulario para actualizar sesiones se usuarios
    def Form_Update_User_Level(self):
        ...
        self.label_user_name.grid(row=0, column=0, padx=10, pady=5)
        self.entry_user_name = tk.Entry(self.top_level, validate="key", validatecommand=self.validate_str)
        self.entry_user_name.insert(0, data_treeview[1])
        self.entry_user_name.grid(row=0, column=1, padx=10, pady=5)

        self.label_password = tk.Label(self.top_level, text="Contraseña:")
        self.label_password.grid(row=1, column=0, padx=10, pady=5)
        self.entry_password = tk.Entry(self.top_level, validate="key", validatecommand=self.validate_str)
        self.entry_password.insert(0, data_treeview[2])
        self.entry_password.grid(row=1, column=1, padx=10, pady=5)
        ...
        self.entry_user_id = ttk.Combobox(self.top_level, values=data_user, state="readonly")
        self.entry_user_id.set(data_treeview[5])
        self.entry_user_id.grid(row=2, column=1, padx=10, pady=5)
        ...
        self.label_rol_id.grid(row=3, column=0, padx=10, pady=5)
        self.entry_rol_id = ttk.Combobox(self.top_level, values=data_rol, state="readonly")
        self.entry_rol_id.set(data_treeview[6])
        self.entry_rol_id.grid(row=3, column=1, padx=10, pady=5)

        button_add = tk.Button(self.top_level, text="Agregar", command=self.Query_Update_User_Level)
        button_add.grid(row=4, columnspan=2, padx=10, pady=10)

    def Query_Update_User_Level(self):
        if self.entry_user_name.get() == "" and self.entry_password == "":
            messagebox.showwarning("Advertencia de Sesion de Usuario", "Debe llenar todos los formularios")
            return
        else:
            if self.control_User_Login.update_user_access_Controller(self.user_acces_id, self.entry_user_name.get(),  self.entry_password.get(), self.entry_user_id.get().split()[0], self.entry_rol_id.get().split()[0]) != None:
    ...
    def Form_Delete_User_Level(self):
        # Verificar que hallá seleccionado una entidad del treeview
        data_treeview = Validaciones.verify_treeview(self.treeview)
        if data_treeview == None:
            messagebox.showwarning("Advertencia del sistema", "Debe seleccionar una fila de la tabla")
            ...
            if messagebox.askyesno("Pregunta del sistema", f"Esta seguro de eliminar el registro\n{data_treeview[0]} - {data_treeview[1]}"):
                self.Query_Delete_User_Level(data_treeview[0])
            else: messagebox.showinfo("Mensaje del sistema", "Se ha revertido con exito")

    def Query_Delete_User_Level(self, id_user_access):
        if self.control_User_Login.delete_user_access_Controller(id_user_access) != None:
